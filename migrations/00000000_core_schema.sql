...
create table if not exists public.profiles (
  ...
  referral_code text,
  buddy_pass_weeks_available integer not null default 0,
  buddy_pass_weeks_earned integer not null default 0,
  buddy_pass_weeks_used integer not null default 0,
...
begin
  ...
  values (
    ...
    coalesce(new.email, ''),
    'free',
    'RM' || upper(substr(md5(new.id::text || coalesce(new.email, '')), 1, 8))
  )
